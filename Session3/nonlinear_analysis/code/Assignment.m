...
for k = 1:length(chill)
    ...
    alpha = zeros(1,length(1:2:length(rangeshort)));
    for i = 2:length(rangeshort)
        alpha(i) = (log10(Fshort(i))-log10(Fshort(i-1)))/(log(i+rangeshort(1)-1)-log(i+rangeshort(1)-2));
    end
    A = [log10(rangeshort)' ones(length(rangeshort),1)];
    b = log10(Fshort)';
    pshort = A\b;
end
disp("Mean Alpha")
...
disp(p(1))
figure
plot(1:length(Fshort),Fshort)
title('F')
figure
plot(1:length(alpha),alpha)
title('Alpha')
figure
plot(log10(rangeshort),log10(Fshort))
xlabel('log(n)')
ylabel('log(F)')

%%
slope = freqdropoff(RR);
%%
[sd1, sd2] = poincare(RR);
